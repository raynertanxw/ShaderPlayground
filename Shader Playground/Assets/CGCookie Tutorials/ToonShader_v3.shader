Shader "_Custom/CGCookie Tutorials/Toon Shader v3"{
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			vertexOutput vert(vertexInput v){
				...
				//light attenuation
				o.atten = lerp( 1.0, 1.0/length(_WorldSpaceLightPos0.xyz - posWorld.xyz), _WorldSpaceLightPos0.w );
			...
			fixed4 frag(vertexOutput i) : COLOR
			{
				...
				fixed3 diffuseReflection = i.atten * _LightColor0.xyz * (1 - specularCutoff) * _Color.xyz * diffuseCutoff;
		...
		Pass {
			...
			fixed4 frag(vertexOutput i) : COLOR
			{
				...
				fixed3 lightFinal = (ambientLight + diffuseReflection) + specularReflection * i.atten;
